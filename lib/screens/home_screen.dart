...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _verificarPerfil() async {
    ...
    try {
      ...
      if (snap.docs.isNotEmpty) {
        ...
        } else if (cargo == 'professor' || emailLogado.contains('admin') || emailLogado.contains('professor')) {
          setState(() {
            _eAdminGeral = false;
            _eProfessor = true;
            _alunoSelecionadoId = null;
  ...
  void _finalizarCronometro() {
    _descansoTimer?.cancel();
    setState(() {
      _cronorchetoAtivo = false;
    });
    
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Descanso Concluído para: $_exercicioEmDescanso! Hora da próxima série! 🔥'),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                const Text('Tempo de Descanso', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 12)),
                Text(_exercicioEmDescanso, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.black87, fontSize: 13, fontWeight: FontWeight.w500)),
              ],
            ),
          ),
          IconButton(
            icon: const Icon(Icons.remove_circle_outline, color: Colors.black87, size: 22),
            onPressed: () {
              setState(() {
          ...
          IconButton(
            icon: const Icon(Icons.add_circle_outline, color: Colors.black87, size: 22),
            onPressed: () {
              setState(() {
                _tempoRestanteSegundos += 15;
                _tempoDefinidoPadrao = _tempoRestanteSegundos;
  ...
  void _cadastrarNovoProfessor() async {
    if (_profNomeController.text.isEmpty || _profEmailController.text.isEmpty || _profSenhaController.text.isEmpty) return;
    
    if (_profSenhaController.text.trim().length < 6) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('A senha do professor deve conter pelo menos 6 caracteres! ❌'), backgroundColor: Colors.redAccent),
      );
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Professor credenciado com sucesso! O Admin permanece ativo. 🛡️'), backgroundColor: Colors.blue),
      );
      ...
      builder: (context) => AlertDialog(
        title: const Row(
          children: [
            ...
            SizedBox(width: 8),
            Text('Editar Cadastro', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          ],
        ),
            ...
            child: Column(
              ...
              children: [
                Center(
                  child: Column(
                    children: [
                      _fotoBase64Edicao != '---' && _fotoBase64Edicao!.isNotEmpty
                          ? CircleAvatar(radius: 45, backgroundImage: MemoryImage(base64Decode(_fotoBase64Edicao!)))
                          : const CircleAvatar(radius: 45, backgroundColor: Colors.black26, child: Icon(Icons.person, size: 40, color: Colors.white)),
                      TextButton.icon(
                        icon: const Icon(Icons.camera_alt, color: Colors.orange),
                        label: const Text('Atualizar Foto do Aluno', style: TextStyle(color: Colors.orange)),
                        onPressed: () async {
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(labelText: 'Número de Celular', hintText: '(11) 99999-9999', border: OutlineInputBorder()),
                ),
              ],
            ),
          ),
  ...
  void _confirmarExclusaoAluno(String idAluno, String nomeAluno) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Tem certeza que deseja apagar permanentemente o cadastro de "$nomeAluno"? Todos os treinos vinculados serão perdidos.'),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              await FirebaseFirestore.instance.collection('usuarios').doc(idAluno).delete();
              ...
              Navigator.pop(context);
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Aluno removido do sistema! 🗑️'), backgroundColor: Colors.redAccent),
              );
            },
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            TextField(controller: _editAnual, decoration: const InputDecoration(labelText: 'Plano Anual')),
          ],
        ),
        actions: [
  ...
  void _abrirGerenciamentoFinanceiro(String clientID, String clientName, Map<String, dynamic> currentData) {
    _planoSelecionadoAluno = currentData['plano'] ?? 'Mensal';
    _mensalidadeController.text = (currentData['valorMensalidade'] ?? currentData['valor'] ?? currentData['mensalidade'] ?? _valoresPlanosCarregados[_planoSelecionadoAluno]).toString();
    _vencimentoController.text = (currentData['diaVencimento'] ?? 10).toString();
        ...
        builder: (context, setPopupState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              TextField(controller: _vencimentoController, decoration: const InputDecoration(labelText: 'Vencimento (Dia)')),
              const SizedBox(height: 10),
              DropdownButtonFormField<String>(
                value: _statusPagamentoSelecionado,
                items: ['Pago', 'Pendente', 'Atrasado'].map((s) => DropdownMenuItem(value: s, child: Text(s))).toList(),
                onChanged: (v) => setPopupState(() => _statusPagamentoSelecionado = v!),
              ),
            ],
          ),
          actions: [
            ElevatedButton(
              onPressed: () async {
                await FirebaseFirestore.instance.collection('usuarios').doc(clientID).update({
                  ...
                  child: Row(
                    ...
                    children: [
                      TextButton.icon(
                        ...
                        label: const Text('Fechar Treino', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
                      ),
                    ],
                  ),
                ),
  ...
  void _adicionarExercicioAoCardapio() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Exercício salvo na biblioteca! ✔'), backgroundColor: Colors.green),
    );
  }
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text('Exercício: $exercise', style: const TextStyle(fontWeight: FontWeight.bold)),
            ...
            TextField(controller: _videoUrlController, decoration: const InputDecoration(labelText: 'Link do Vídeo (Opcional)')),
          ],
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancelar')),
  ...
  void _salvarNovaCargaHistorico(String nomeExercicio, String novaCarga) async {
    ...
    Navigator.pop(context);
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Nova carga registrada com sucesso! 💪'), backgroundColor: Colors.green),
    );
  }
  ...
  void _matricularNovoAluno() async {
    ...
    if (_novaSenhaController.text.trim().length < 6) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('A senha do aluno deve conter pelo menos 6 caracteres! ❌'), backgroundColor: Colors.redAccent),
      );
      return;
    }
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('usuarios').doc(novoUidDoAluno).set({
        ...
        'celular': _novoCelularController.text.trim().isEmpty ? '---' : _novoCelularController.text.trim(),
      ...
      Navigator.pop(context);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Aluno matriculado com sucesso! Painel do professor permanece intacto. 🚀'), backgroundColor: Colors.green),
      );
    } on FirebaseAuthException catch (e) {
        ...
        builder: (context, setPopupState) => AlertDialog(
          ...
          title: const Row(
            children: [
              ...
              SizedBox(width: 8),
              Text('Copiloto IA: Montar Ficha', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            ],
          ),
          content: SingleChildScrollView(
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Defina o perfil do aluno para o Gemini sugerir a ficha:', style: TextStyle(fontSize: 13, color: Colors.blueGrey)),
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(
                  value: objetivoSelecionado,
                  decoration: const InputDecoration(labelText: 'Objetivo Principal', border: OutlineInputBorder()),
                  items: ['Hipertrofia', 'Emagrecimento / Definição', 'Resistência Muscular', 'Força Máxima'].map((o) => DropdownMenuItem(value: o, child: Text(o))).toList(),
                  onChanged: (v) => setPopupState(() => objetivoSelecionado = v!),
                ),
                const SizedBox(height: 10),
                DropdownButtonFormField<String>(
                  value: nivelSelecionado,
                  decoration: const InputDecoration(labelText: 'Nível Atual', border: OutlineInputBorder()),
                ...
                ElevatedButton(
                  ...
                  onPressed: processandoIA || gravandoNoBanco ? null : () async {
                    ...
                    final resposta = await _aiService.gerarSugestaoTreino(
                      objetivo: '$objetivoSelecionado ($diretrizMetodologica. ATENÇÃO: Devolva a resposta estritamente formatada em JSON válido. O formato deve ser uma lista de objetos contendo as chaves: "ficha", "grupo", "exercicio", "series", "carga".)',
                  ...
                  child: processandoIA 
                    ...
                    : const Text('Consultar Gemini IA 🚀', style: TextStyle(fontWeight: FontWeight.bold)),
                ),
                if (resultadoIA.isNotEmpty) ...[
                  const Padding(padding: EdgeInsets.symmetric(vertical: 10), child: Divider()),
                  const Text('Sugestão Estruturada da IA:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.black)),
                  ...
                  Container(
                    ...
                    child: SingleChildScrollView(
                      child: Text(resultadoIA, style: const TextStyle(fontSize: 12, fontFamily: 'monospace', color: Colors.black87)),
                    ),
                  ),
                  const SizedBox(height: 12),
                  ElevatedButton.icon(
                    style: ElevatedButton.styleFrom(backgroundColor: Colors.green[700], foregroundColor: Colors.white, minimumSize: const Size.fromHeight(42)),
                    icon: gravandoNoBanco 
                        ? const SizedBox(height: 18, width: 18, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2))
                        : const Icon(Icons.playlist_add_check),
                    label: Text(gravandoNoBanco ? 'Gravando na Grade...' : 'Injetar Exercícios na Ficha do Aluno 🔥'),
                    onPressed: gravandoNoBanco || _alunoSelecionadoId == null ? null : () async {
                      setPopupState(() => gravandoNoBanco = true);
                      try {
                        String marcaInicioJson = '```json';
                        String marcaFimJson = '
```';
                        String limpandoJson = resultadoIA.replaceAll(marcaInicioJson, '').replaceAll(marcaFimJson, '').trim();
                        final List<dynamic> listaExerciciosIA = jsonDecode(limpandoJson);

                        for (var exercicioItem in listaExerciciosIA) {
                          String letraFicha = (exercicioItem['ficha'] ?? _fichaSelecionadaAluno).toString().toUpperCase().trim();
                          String grupoMusc = exercicioItem['grupo'] ?? 'Geral';
                          String nomeEx = exercicioItem['exercicio'] ?? 'Exercício';
                          ...
                          await _workoutService.salvarTreino(
                            ...
                            carga: peso.contains('kg') ? peso : '$peso kg',
                            videoUrl: '---',
                          );
                        }
                        ...
                        Navigator.pop(context);
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Ficha Inteligente de $_alunoSelecionadoNome montada com sucesso! 🏋️‍♂️💪'), backgroundColor: Colors.green[800]),
                        );
                      } catch (erroParsing) {
                        print("Erro ao processar estrutura JSON da IA: $erroParsing");
                        setPopupState(() => gravandoNoBanco = false);
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Falha ao processar o formato gerado. Tente consultar novamente.'), backgroundColor: Colors.redAccent),
                        );
                      }
                    },
                  ),
  ...
  void _injetarPlanoTreinoPadrao(String nivel) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Plano Padrão ($nivel) injetado com sucesso na ficha do aluno! 🏋️‍♂️🔥'), backgroundColor: Colors.green[800]),
      );
    } catch (e) {
      Navigator.pop(context);
        ...
        builder: (context, setPopupState) => AlertDialog(
          ...
          title: const Row(
            children: [
              ...
              SizedBox(width: 8),
              Text('Scanner de Refeição IA', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            ],
          ),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              if (!analisando && resultadoAnalise == null) ...[
                const Text('Tire uma foto do seu prato para a IA calcular calorias e proteínas automaticamente.', textAlign: TextAlign.center, style: TextStyle(fontSize: 13, color: Colors.blueGrey)),
                const SizedBox(height: 16),
                ElevatedButton.icon(
                  style: ElevatedButton.styleFrom(backgroundColor: Colors.green[700], foregroundColor: Colors.white, minimumSize: const Size.fromHeight(42)),
                  icon: const Icon(Icons.photo_camera),
                  label: const Text('Bater Foto do Prato', style: TextStyle(fontWeight: FontWeight.bold)),
                  onPressed: () async {
                    ...
                    if (foto != null) {
                      ...
                      final resultado = await _aiService.analisarRefeicaoPorFoto(bytes);
              ...
              if (analisando) ...[
                ...
                const SizedBox(height: 16),
                const Text('O Gemini está analisando os macros do prato...', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500)),
              ],
              if (resultadoAnalise != null) ...[
                Container(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('🍽️ ${resultadoAnalise!['descricao']}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.black87)),
                      const Padding(padding: EdgeInsets.symmetric(vertical: 6), child: Divider(height: 1)),
                      Text('🔥 Calorias: ${resultadoAnalise!['calorias']} kcal', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Colors.black)),
                      const SizedBox(height: 4),
                      Text('💪 Proteínas: ${resultadoAnalise!['proteinas']}g', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Colors.green)),
                    ],
                  ),
                ),
                const SizedBox(height: 16),
                ElevatedButton(
                  style: ElevatedButton.styleFrom(backgroundColor: Colors.black, foregroundColor: Colors.amber, minimumSize: const Size.fromHeight(42)),
                  onPressed: () async {
                    await FirebaseFirestore.instance
                        .collection('usuarios')
                        .doc(_alunoSelecionadoId)
                    ...
                    Navigator.pop(context);
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Refeição salva no seu diário de nutrição! 🍗📊'), backgroundColor: Colors.green),
                    );
                  },
                  child: const Text('Confirmar e Salvar no Diário', style: TextStyle(fontWeight: FontWeight.bold)),
                )
              ],
            ],
          ),
  ...
  // ATUALIZADO: Pop-up comercial com exibição da chave PIX real da academia
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
            ...
            Container(
              ...
              child: const Column(
                children: [
                  ...
                  SizedBox(height: 4),
                  SelectableText(
                    'SUA_CHAVE_PIX_REAL_AQUI', // 👈 Substitua aqui pela chave PIX real da academia
                    textAlign: TextAlign.center,
        ...
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Agora não', style: TextStyle(color: Colors.black)),
          ),
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: Colors.purple, foregroundColor: Colors.white),
            onPressed: () async {
              // Modifica o status para notificar o sistema do professor da transação
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('👍 Confirmado! Seu sinal de pagamento foi enviado. O professor vai conferir o PIX e liberar seu acesso em breve!'),
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            Expanded(child: Text(dados['exercicios'] ?? 'Exercício', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18))),
          ],
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text('Carga Base do Professor: ${dados['carga'] ?? '---'}', style: const TextStyle(fontSize: 16, color: Colors.blueAccent, fontWeight: FontWeight.bold)),
          ],
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Fechar', style: TextStyle(color: Colors.black))),
  ...
  void _abrirPopupAnotarCarga(String nomeExercicio) {
    ...
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Anotar Carga de Hoje', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Exercício: $nomeExercicio', style: const TextStyle(fontSize: 13, color: Colors.blueGrey)),
            const SizedBox(height: 12),
            TextField(
        ...
        actions: [
          ...
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: Colors.black, foregroundColor: Colors.amber),
            onPressed: () => _salvarNovaCargaHistorico(nomeExercicio, _anotarCargaController.text),
          ...
          children: [
            const Row(
              children: [
                ...
                SizedBox(width: 8),
                Text('Evolução de Força', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              ],
            ),
            const SizedBox(height: 4),
            Text(nomeExercicio, style: const TextStyle(fontSize: 13, color: Colors.grey)),
          ],
        ),
        content: SizedBox(
          ...
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapHist) {
              if (!snapHist.hasData) return const Center(child: CircularProgressIndicator(color: Colors.amber));
              ...
              if (logs.isEmpty) {
                return const Center(
                  child: Text('Nenhuma carga anotada para este exercício ainda.', textAlign: TextAlign.center, style: TextStyle(color: Colors.grey, fontSize: 13)),
                );
              }
              ...
              for (int index = 0; index < logs.length; index++) {
                ...
                double valorNumericoCarga = double.tryParse(apenasNumeros) ?? 0.0;

                if (valorNumericoCarga > maiorCargaEncontrada) {
                  maiorCargaEncontrada = valorNumericoCarga;
                }

                pontosDoGrafico.add(FlSpot(index.toDouble(), valorNumericoCarga));
              }

              if (!carregouIA) {
                carregouIA = true;
                _aiService.analisarProgressaoCarga(nomeExercicio: nomeExercicio, historicoCargas: listaPesosOrdenados).then((res) {
                  if (mounted) {
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            analiseIA,
                            style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500, height: 1.4, color: Colors.black87),
                          ),
                        ),
                      ],
                    ),
                      ...
                      child: LineChart(
                        LineChartData(
                          ...
                          borderData: FlBorderData(
                            show: true,
                            border: Border(bottom: BorderSide(color: Colors.grey[300]!, width: 1), left: BorderSide(color: Colors.grey[300]!, width: 1)),
                          ),
                          minX: 0,
                          maxX: logs.length > 1 ? (logs.length - 1).toDouble() : 1,
                          minY: 0,
                          maxY: maiorCargaEncontrada > 0 ? (maiorCargaEncontrada + 10) : 50,
                          lineBarsData: [
                            LineChartBarData(
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, idx) {
                        var logData = logsExibicaoTexto[idx].data() as Map<String, dynamic>;
                        String cargaReg = logData['carga'] ?? '---';
                        
                        String dataFormatada = 'Recente';
                        if (logData['dataAnotacao'] != null) {
                          var dataTime = (logData['dataAnotacao'] as Timestamp).toDate();
                          dataFormatada = '${dataTime.day.toString().padLeft(2, '0')}/${dataTime.month.toString().padLeft(2, '0')} às ${dataTime.hour.toString().padLeft(2, '0')}:${dataTime.minute.toString().padLeft(2, '0')}';
                        }

                        return ListTile(
                          dense: true,
                          leading: const Icon(Icons.fitness_center, size: 16, color: Colors.black87),
                          title: Text(cargaReg, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                          subtitle: Text(dataFormatada, style: const TextStyle(fontSize: 11)),
                        );
                      },
                    ),
                  ),
  ...
  void _removerExercicioDoCardapio(String docId) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Exercício removido da biblioteca.'), backgroundColor: Colors.orange),
    );
  }
  ...
  Widget _construirCardValidadeMensalidade(Map<String, dynamic> dados) {
    ...
    bool estaAtrasado = status == 'Atrasado' || (status == 'Pendente' && diaHoje > diaVencimento);
    bool estaPago = status == 'Pago';

    Color colCard = estaPago ? Colors.green[700]! : (estaAtrasado ? Colors.red[700]! : Colors.orange[700]!);
    IconData icone = estaPago ? Icons.check_circle : (estaAtrasado ? Icons.warning : Icons.hourglass_empty);
            ...
            Row(
              children: [
                ...
                Text(
                  estaPago 
                      ? 'Mensalidade em Dia' 
                      : (estaAtrasado ? 'Atenção: Mensalidade Vencida' : 'Mensalidade Pendente'),
  ...
  Widget _construirMiniCardFaturamento(String titulo, String valor, Color cor) {
    return Expanded(
      child: Container(
        ...
        child: Column(
          children: [
            Text(titulo, textAlign: TextAlign.center, style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: cor.withOpacity(0.9))),
            const SizedBox(height: 4),
            Text(valor, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: cor)),
          ],
        ),
      ),
    );
      ...
      children: [
        if (_eAdminGeral) ...[
          Card(
            ...
            child: ExpansionTile(
              ...
              title: const Text('Área do Dono: Cadastrar Professor', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
              children: [
                Padding(
                  padding: const EdgeInsets.all(12.0),
                  child: Column(
                    children: [
                      TextField(controller: _profNomeController, style: const TextStyle(color: Colors.white), decoration: const InputDecoration(labelText: 'Nome do Professor', labelStyle: TextStyle(color: Colors.white70), enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: Colors.white30)), focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: Colors.amber)))),
                      const SizedBox(height: 8),
                      TextField(controller: _profEmailController, style: const TextStyle(color: Colors.white), decoration: const InputDecoration(labelText: 'E-mail de Acesso', labelStyle: TextStyle(color: Colors.white70), enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: Colors.white30)), focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: Colors.amber)))),
                      const SizedBox(height: 8),
                      TextField(controller: _profSenhaController, obscureText: true, style: const TextStyle(color: Colors.white), decoration: const InputDecoration(labelText: 'Senha Inicial (Mínimo 6 caracteres)', labelStyle: TextStyle(color: Colors.white70), enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: Colors.white30)), focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: Colors.amber)))),
                      const SizedBox(height: 12),
                      ElevatedButton(
                        style: ElevatedButton.styleFrom(backgroundColor: Colors.amber, foregroundColor: Colors.black, minimumSize: const Size.fromHeight(42)),
                        onPressed: _profNomeController.text.isEmpty || _profEmailController.text.isEmpty || _profSenhaController.text.isEmpty ? null : _cadastrarNovoProfessor,
                        child: const Text('Autorizar e Cadastrar Professor', style: TextStyle(fontWeight: FontWeight.bold)),
                      ),
                    ],
                  ),
                )
        ...
        Card(
          ...
          child: ExpansionTile(
            ...
            title: const Text('Gerenciar Biblioteca de Exercícios', style: TextStyle(fontWeight: FontWeight.bold)),
            children: [
              Padding(
                padding: const EdgeInsets.all(12.0),
                child: Column(
                  ...
                  children: [
                    DropdownButtonFormField<String>(
                      ...
                      decoration: const InputDecoration(labelText: 'Grupo Muscular', border: OutlineInputBorder()),
                      items: _gruposMusculares.map((g) => DropdownMenuItem(value: g, child: Text(g))).toList(),
                      onChanged: (val) => setState(() => _grupoSelecionadoParaNovoExercicio = val!),
                    ),
                    const SizedBox(height: 8),
                    TextField(controller: _novoExercicioCardapioController, decoration: const InputDecoration(labelText: 'Nome do Exercício (Ex: Supino Reto)', border: OutlineInputBorder())),
                    const SizedBox(height: 8),
                    TextField(controller: _videoUrlController, decoration: const InputDecoration(labelText: 'Link do Vídeo no YouTube', border: OutlineInputBorder())),
                    const SizedBox(height: 12),
                    ElevatedButton(
                      style: ElevatedButton.styleFrom(backgroundColor: Colors.blueAccent, foregroundColor: Colors.white, minimumSize: const Size.fromHeight(42)),
                      onPressed: _adicionarExercicioAoCardapio,
                      child: const Text('Salvar na Biblioteca'),
                    ),
                    const Divider(height: 24),
                    const Text('Exercícios Salvos:', style: TextStyle(fontWeight: FontWeight.bold)),
                    const SizedBox(height: 6),
                    
                    StreamBuilder<QuerySnapshot>(
                      stream: FirebaseFirestore.instance.collection('exercicios').orderBy('criadoEm', descending: true).snapshots(),
                      builder: (context, snap) {
                        if (!snap.hasData) return const SizedBox();
                        var lista = snap.data!.docs;
                        if (lista.isEmpty) return const Text('Nenhum exercício armazenado ainda.', style: TextStyle(fontSize: 12, color: Colors.grey));
                        return SizedBox(
                          height: 180,
                          child: ListView.builder(
                            shrinkWrap: true,
                            ...
                            itemBuilder: (context, i) {
                              var item = lista[i].data() as Map<String, dynamic>;
                              return ListTile(
                                dense: true,
                                ...
                                trailing: IconButton(
                                  ...
                                  onPressed: () => _removerExercicioDoCardapio(lista[i].id),
                                ),
                              );
                            },
                          ),
        ...
        Card(
          ...
          child: ExpansionTile(
            ...
            title: const Text('Matricular Novo Aluno', style: TextStyle(fontWeight: FontWeight.bold)),
            children: [
              Padding(
                padding: const EdgeInsets.all(12.0),
                child: Column(
                  children: [
                    Row(
                      ...
                      children: [
                        _fotoBase64NovaMatricula != null
                            ? CircleAvatar(radius: 36, backgroundImage: MemoryImage(base64Decode(_fotoBase64NovaMatricula!)))
                            : const CircleAvatar(radius: 36, backgroundColor: Colors.black12, child: Icon(Icons.camera_enhance, color: Colors.black38)),
                        const SizedBox(width: 14),
                        ElevatedButton.icon(
                          style: ElevatedButton.styleFrom(backgroundColor: Colors.grey[800], foregroundColor: Colors.amber),
                    ...
                    const SizedBox(height: 12),
                    TextField(controller: _novoNomeController, decoration: const InputDecoration(labelText: 'Nome Completo', border: OutlineInputBorder())),
                    const SizedBox(height: 8),
                    TextField(controller: _novoEmailController, decoration: const InputDecoration(labelText: 'E-mail', border: OutlineInputBorder())),
                    const SizedBox(height: 8),
                    TextField(controller: _novaSenhaController, obscureText: true, decoration: const InputDecoration(labelText: 'Senha de Acesso (Mínimo 6 caracteres)', border: OutlineInputBorder())),
                    const SizedBox(height: 8),
                    TextField(controller: _novoCelularController, keyboardType: TextInputType.phone, decoration: const InputDecoration(labelText: 'Celular/WhatsApp', hintText: '(11) 99999-9999', border: OutlineInputBorder())),
                    const SizedBox(height: 12),
                    ElevatedButton(
                      style: ElevatedButton.styleFrom(backgroundColor: Colors.black, foregroundColor: Colors.amber, minimumSize: const Size.fromHeight(45)),
        ...
        TextField(
          onChanged: (val) => setState(() => _filtroBuscaAlunos = val.toLowerCase().trim()),
        ...
        const Text('Lista de Alunos Cadastrados:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        const SizedBox(height: 8),
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    leading: fotoSalvaMemory != '---' && fotoSalvaMemory.isNotEmpty
                        ? CircleAvatar(
                            backgroundImage: MemoryImage(base64Decode(fotoSalvaMemory)),
                            backgroundColor: Colors.black,
                          )
                        : CircleAvatar(
                            backgroundColor: Colors.black, 
                            foregroundColor: Colors.amber, 
                            child: Text(dados['nome']?[0].toString().toUpperCase() ?? 'A')
                          ),
                    title: Text(dados['nome'] ?? 'Sem nome', style: const TextStyle(fontWeight: FontWeight.bold)),
                    subtitle: Text('Tel: $celularAluno | Vence dia: ${dados['diaVencimento'] ?? 10}'),
                    trailing: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        IconButton(
                        ...
                        IconButton(
                          ...
                          onPressed: () {
                            _confirmarExclusaoAluno(doc.id, dados['nome'] ?? 'Aluno');
                          },
                        ),
                        if (selecionado) const Icon(Icons.check_circle, color: Colors.black)
                      ],
                    ),
                    onTap: () {
                      setState(() {
          ...
          StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapGlobalEx) {
              ...
              if (snapGlobalEx.hasData) {
                exerciciosTexto = snapGlobalEx.data!.docs.map((e) => (e.data() as Map)['nome'].toString()).toList();
              }
              return Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Row(
                    ...
                    children: [
                      WidgetExpandido(
                        child: Text('Treino de: $_alunoSelecionadoNome', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.blueGrey, fontSize: 15)),
                      ),
                      ElevatedButton.icon(
                        style: ElevatedButton.styleFrom(backgroundColor: Colors.purple[700], foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4)),
                        icon: const Icon(Icons.auto_awesome, size: 16),
                        label: const Text('Copiloto IA ✨', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                  ...
                  Card(
                    ...
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10), side: BorderSide(color: Colors.blueGrey[200]!)),
                    child: Padding(
                      padding: const EdgeInsets.all(12.0),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Row(
                            children: [
                              Icon(Icons.copy, color: Colors.blueGrey, size: 18),
                              SizedBox(width: 6),
                              Text('Injetar Modelo de Treino Padrão', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.black87)),
                            ],
                          ),
                          const SizedBox(height: 10),
                          Row(
                            children: [
                              Expanded(
                                child: ElevatedButton(
                                  style: ElevatedButton.styleFrom(backgroundColor: Colors.green[600], foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 8)),
                                  onPressed: () => _injetarPlanoTreinoPadrao('Iniciante'),
                                  child: const Text('Iniciante 🟢', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                                ),
                              ),
                              const SizedBox(width: 6),
                              Expanded(
                                child: ElevatedButton(
                                  style: ElevatedButton.styleFrom(backgroundColor: Colors.orange[700], foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 8)),
                                  onPressed: () => _injetarPlanoTreinoPadrao('Intermediário'),
                                  child: const Text('Intermediário 🟡', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                                ),
                              ),
                              const SizedBox(width: 6),
                              Expanded(
                                child: ElevatedButton(
                                  style: ElevatedButton.styleFrom(backgroundColor: Colors.red[700], foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 8)),
                                  onPressed: () => _injetarPlanoTreinoPadrao('Avançado'),
                                  child: const Text('Avançado 🔴', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                                ),
                              ),
                            ],
                          ),
          ...
          Card(
            ...
            child: Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Escolha pela Biblioteca Armazenada', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blueAccent)),
                  const SizedBox(height: 12),
                  
                  DropdownButtonFormField<String>(
                    value: _grupoSelecionadoFiltro,
                    decoration: const InputDecoration(labelText: '1. Selecione o Grupo Muscular', border: OutlineInputBorder()),
                    items: _gruposMusculares.map((g) => DropdownMenuItem(value: g, child: Text(g))).toList(),
                    onChanged: (val) {
                      setState(() {
                  ...
                  StreamBuilder<QuerySnapshot>(
                    stream: FirebaseFirestore.instance.collection('exercicios').where('grupo', isEqualTo: _grupoSelecionadoFiltro).snapshots(),
                    builder: (context, snapEx) {
                      if (!snapEx.hasData) return const LinearProgressIndicator();
                      var docsEx = snapEx.data!.docs;

                      if (docsEx.isEmpty) {
                        return const Padding(
                          padding: EdgeInsets.symmetric(vertical: 8.0),
                          child: Text('Nenhum exercício deste grupo salvo na biblioteca.', style: TextStyle(color: Colors.red, fontSize: 13)),
                        );
                      }

                      // PROTEÇÃO ABSOLUTA DROPDOWN
                      final chavesValidas = docsEx.map((de) {
                        var exData = de.data() as Map<String, dynamic>;
                        return "${exData['nome'] ?? ''}|||${exData['videoUrl'] ?? '---'}";
                      }).toSet().toList();

                      if (_exercicioSelecionadoCardapio != null && !chavesValidas.contains(_exercicioSelecionadoCardapio)) {
                      ...
                      return DropdownButtonFormField<String>(
                        value: chavesValidas.contains(_exercicioSelecionadoCardapio) ? _exercicioSelecionadoCardapio : null,
                        ...
                        onChanged: (val) => setState(() => _exercicioSelecionadoCardapio = val),
                      );
                    },
                  ),
                  const SizedBox(height: 14),
                  ElevatedButton.icon(
                    ...
                    onPressed: _exercicioSelecionadoCardapio == null 
                      ...
                      : () {
                          ...
                          _abrirConfiguracaoExercicio(_grupoSelecionadoFiltro, nomeEx, videoUrlAtual: urlVid);
                        },
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData && snapshot.data != null) {
          for (var doc in snapshot.data!.docs) {
            ...
            String cargo = (d['cargo'] ?? 'aluno').toString().toLowerCase().trim();
            if (cargo == 'aluno') {
              alunosFiltrados.add(doc);
              
              double valor = 0.0;
              if (d['valorMensalidade'] != null) {
                valor = double.tryParse(d['valorMensalidade'].toString()) ?? 0.0;
              ...
              String status = (d['statusPagamento'] ?? 'Pendente').toString().trim();
              int diaVencimento = int.tryParse(d['diaVencimento'].toString()) ?? 10;
              int diaHoje = DateTime.now().day;

              if (status.toLowerCase() == 'pago') {
                faturamentoPago += valor;
              } else if (status.toLowerCase() == 'atrasado' || (status.toLowerCase() == 'pendente' && diaHoje > diaVencimento)) {
        ...
        return Column(
          ...
          children: [
                ...
                child: Column(
                  ...
                  children: [
                    const Text(
                      ...
                      style: TextStyle(fontWeight: FontWeight.w800, fontSize: 14, color: Colors.blueGrey)
                    ),
                    const SizedBox(height: 12),
                    Row(
                      children: [
                        _construirMiniCardFaturamento('Arrecadado', 'R\$ ${faturamentoPago.toStringAsFixed(2)}', Colors.green[700]!),
                        const SizedBox(width: 8),
                        _construirMiniCardFaturamento('A Receber', 'R\$ ${faturamentoPendente.toStringAsFixed(2)}', Colors.orange[700]!),
                        const SizedBox(width: 8),
                        _construirMiniCardFaturamento('Inadimplente', 'R\$ ${faturamentoAtrasado.toStringAsFixed(2)}', Colors.red[700]!),
                      ],
                    ),
                  ],
                ),
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                const Text('Status de Mensalidades Individuais', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                IconButton(icon: const Icon(Icons.settings, color: Colors.black), onPressed: _abrirConfiguracaoDeValoresDoCardapio)
              ],
            ),
            const SizedBox(height: 8),
            ...
            if (alunosFiltrados.isEmpty) 
              const Padding(
                ...
                child: Center(child: Text('Nenhum aluno ativo para faturamento.')),
              )
            else
              ListView.builder(
                shrinkWrap: true,
                ...
                itemBuilder: (context, index) {
                  ...
                  double valorExibicao = double.tryParse((dados['valorMensalidade'] ?? dados['valor'] ?? dados['mensalidade'] ?? 0).toString()) ?? 0.0;

                  return Card(
                    color: Colors.white,
                    child: ListTile(
                      title: Text(dados['nome'] ?? 'Aluno', style: const TextStyle(fontWeight: FontWeight.bold)),
                      subtitle: Text('Plano: ${dados['plano'] ?? 'Mensal'} | Valor: R\$ ${valorExibicao.toStringAsFixed(2)}'),
                      trailing: Container(
                        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                        decoration: BoxDecoration(color: _obterCorStatus(status).withOpacity(0.2), borderRadius: BorderRadius.circular(8)),
                        child: Text(status, style: TextStyle(color: _obterCorStatus(status), fontWeight: FontWeight.bold)),
                      ),
                      onTap: () => _abrirGerenciamentoFinanceiro(doc.id, dados['nome'] ?? 'Aluno', dados),
                    ),
                  );
                },
              ),
        ...
        title: Text(
          _eProfessor
              ? (_abaAtualProfessor == 0 
                  ? (_eAdminGeral ? 'Painel Admin Geral' : 'Painel do Professor') 
                  : 'Gestão Financeira 💰')
              : 'Meus Treinos',
        ),
        actions: [
            ...
            : Column(
                ...
                children: [
                  ...
                  StreamBuilder<DocumentSnapshot>(
                    stream: FirebaseFirestore.instance.collection('usuarios').doc(_alunoSelecionadoId).snapshots(),
                    builder: (context, snapUser) {
                      if (!snapUser.hasData || !snapUser.data!.exists) return const SizedBox();
                      var dadosCompletosAluno = snapUser.data!.data() as Map<String, dynamic>;
                      return Padding(
                        padding: const EdgeInsets.only(bottom: 16.0),
                        child: _construirCardValidadeMensalidade(dadosCompletosAluno),
                      );
                    },
                  ),
                  ...
                  StreamBuilder<DocumentSnapshot>(
                    ...
                    builder: (context, snapUser) {
                      ...
                      var dadosAluno = snapUser.data!.data() as Map<String, dynamic>;
                      String iaStatus = dadosAluno['iaStatusAssinatura'] ?? 'NaoAtivado'; // NaoAtivado, Degustacao, Bloqueado, PendenteAprovacao, Ativo
                      Timestamp? dataInicioTimestamp = dadosAluno['dataInicioDegustacao'] as Timestamp?;
                      
                      int diasRestantes = 0;
                      bool emDegustacaoValida = false;

                      if (iaStatus == 'Degustacao' && dataInicioTimestamp != null) {
                        DateTime dataInicio = dataInicioTimestamp.toDate();
                        DateTime dataFimDegustacao = dataInicio.add(const Duration(days: 7));
                        DateTime hoje = DateTime.now();
                        
                        diasRestantes = dataFimDegustacao.difference(hoje).inDays + 1;
                        
                        if (hoje.isBefore(dataFimDegustacao) && diasRestantes >= 0) {
                          emDegustacaoValida = true;
                        } else {
                          emDegustacaoValida = false;
                          FirebaseFirestore.instance.collection('usuarios').doc(_alunoSelecionadoId).update({
                            'iaStatusAssinatura': 'Bloqueado',
                          });
                          iaStatus = 'Bloqueado';
                        }
                      }

                      bool temAcessoLiberado = (iaStatus == 'Ativo') || emDegustacaoValida;
                      ...
                      return Card(
                        ...
                        child: Padding(
                          ...
                          child: Row(
                            children: [
                              CircleAvatar(
                                backgroundColor: temAcessoLiberado ? Colors.green[100] : Colors.purple[100],
                                radius: 22,
                                child: Icon(Icons.restaurant, color: temAcessoLiberado ? Colors.green[800] : Colors.purple[800], size: 22),
                              ),
                              const SizedBox(width: 14),
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        const Text('Diário de Nutrição com IA', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                                        const SizedBox(width: 6),
                                        if (emDegustacaoValida)
                                          Container(
                                            padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                                            decoration: BoxDecoration(color: Colors.amber[100], borderRadius: BorderRadius.circular(6)),
                                            child: Text('$diasRestantes dias grátis', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.amber[900])),
                                          ),
                                        if (iaStatus == 'Ativo')
                                          Container(
                                            padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                                            decoration: BoxDecoration(color: Colors.green[100], borderRadius: BorderRadius.circular(6)),
                                            child: const Text('PRO', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.green)),
                                          ),
                                        if (iaStatus == 'PendenteAprovacao')
                                          Container(
                                            padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                                            decoration: BoxDecoration(color: Colors.orange[100], borderRadius: BorderRadius.circular(6)),
                                            child: Text('Análise PIX', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.orange[800])),
                                          ),
                                      ],
                                    ),
                                ...
                                ElevatedButton(
                                  ...
                                  onPressed: () async {
                                    await FirebaseFirestore.instance.collection('usuarios').doc(_alunoSelecionadoId).update({
                                      ...
                                      'dataInicioDegustacao': FieldValue.serverTimestamp(),
                                    });
                                    if (!mounted) return;
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(content: Text('🎉 Seus 7 dias de degustação grátis começaram! Aproveite o Scanner IA.'), backgroundColor: Colors.purple),
                                    );
                                  },
                                  child: const Text('Testar Grátis', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                                ),
                              if (temAcessoLiberado)
                                ElevatedButton.icon(
                                  ...
                                  label: const Text('Escanear', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                              ...
                              if (iaStatus == 'Bloqueado')
                                ElevatedButton(
                                  ...
                                  child: const Text('Liberar IA 🚀', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                                ),
                              if (iaStatus == 'PendenteAprovacao')
                                ElevatedButton(
                                  ...
                                  onPressed: () {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(content: Text('Sinal já enviado! O professor está checando o extrato para liberar o seu acesso PRO. 🔥')),
                                    );
                                  },
                                  child: const Text('Pendente', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                                ),
                            ],
                          ),
                        ),
  ...
  Widget _construirListaDeTreinosEfetivos() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshotGeralTreinos) {
        Set<String> letrasFichasExistentes = {'A', 'B', 'C', _fichaSelecionadaAluno};
        ...
        List<String> listaFichasOrdenadas = letrasFichasExistentes.toList()..sort();
                  ...
                  Row(
                    children: [
                      TextButton.icon(
                        ...
                        label: const Text('Nova Ficha', style: TextStyle(color: Colors.blueAccent, fontSize: 13, fontWeight: FontWeight.bold)),
                        onPressed: () {
                          int proximoCodigoLetra = listaFichasOrdenadas.last.codeUnitAt(0) + 1;
                          String proximaLetra = String.fromCharCode(proximoCodigoLetra);
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('Ficha $proximaLetra liberada! Agora basta vincular exercícios a ela. 🏋️‍♂️'), backgroundColor: Colors.blue),
                          );
                        },
                      ),
                      IconButton(
              ...
              builder: (context, snapshot) {
                ...
                return Column(
                  children: treinos.map((t) {
                    ...
                    String nomeDoExercicioItem = dadosTratados['exercicios'] ?? 'Exercício';
                              ...
                              children: [
                                ...
                                if (_eProfessor) ...[
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () => _abrirPopupAnotarCarga(nomeExercicioItem),
                                  ),
                                  IconButton(
